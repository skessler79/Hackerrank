...
string organizingContainers(vector<vector<int>> container) {
    ...
    vector<int> ball_count(size);
    vector<int> ball_per_container(size);
    ...
    for(int i = 0; i < size; ++i)
    {
        ...
        ball_per_container[i] = sum;
    }

    sort(ball_count.begin(), ball_count.end());
    sort(ball_per_container.begin(), ball_per_container.end());

    if(ball_count == ball_per_container)
        return "Possible";
    return "Impossible";
}
