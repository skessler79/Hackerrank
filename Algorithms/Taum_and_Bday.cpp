...
long long taumBday(int b, int w, int bc, int wc, int z) {
    if(bc > wc)
    {
        if((long long)bc > (long long)wc + z)
        {
            return (long long)w * wc + b * ((long long)wc + z);
        }
        else
        {
            return (long long)w * wc + (long long)b * bc;
        }
    }
    else
    {
        if((long long)wc > (long long)bc + z)
        {
            return (long long)b * bc + w * ((long long)bc + z);
        }
        else
        {
            return (long long)w * wc + (long long)b * bc;
